...
## 1. 目的
LINE Messaging APIを通じてユーザーから送信されたテキストまたは画像を元に、AIを用いて「入力モチーフに沿った可愛いおかし画像」を生成し、リアルタイムWebアプリに反映させるためのデータベース（Firebase）へ格納する。
...
- メッセージタイプが `text` または `image` であることを確認する。
- `text` の場合は文字列を抽出する。
- `image` の場合は `messageId` を使ってLINE Content APIから元画像バイナリを取得する。

### ② 画像生成プロンプト構築
- ユーザー入力をAIが理解しやすい英語プロンプトに変換。
- `text` 入力時は、入力文字列をモチーフ化するプロンプトを構築する。
- `image` 入力時は、入力写真の被写体をモチーフ化するマルチモーダル入力（画像 + 指示文）を構築する。
...
- `image` 入力時は、入力画像とテキスト指示を同時に渡して生成する。
...
### ⑦ 画像入力（写真）対応
- LINEの `image` メッセージ受信時、LINE Content APIで画像バイナリを取得する。
- 取得した画像をGenAIへ入力し、被写体に似たモチーフのお菓子画像を生成する。
- 生成後の処理（白背景透過化、Storage保存、Realtime Database登録、LINE返信）は `text` 入力時と同一フローで実行する。